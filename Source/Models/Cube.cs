...
namespace Cubicle.Models {
    static class Cube {
        ...
        static Vector3 FaceNormal(VertexPositionNormalTexture[] vertices) {
            var f = (vertices[1].Position - vertices[0].Position);
            var s = (vertices[2].Position - vertices[0].Position);
            return Vector3.Cross(new Vector3(f.X, f.Y, f.Z), new Vector3(s.X, s.Y, s.Z));
        }
    }
}
